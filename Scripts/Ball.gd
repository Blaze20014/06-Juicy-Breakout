...
onready var paddHit = get_node("/root/Game/PaddHit")
...
func _process(delta):
	...
	temp.color = temp.color.linear_interpolate(Color(0, 0, 0, 1), .5)
...
func _apply_shake():
	...
	var o_x = _max_offset * shake * get_neg_or_pos_scalar()
	var o_y = _max_offset * shake * get_neg_or_pos_scalar()
	$ColorRect.rect_position = _start_position + Vector2(o_x, o_y)
	
func get_neg_or_pos_scalar():
